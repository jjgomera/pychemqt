...
class pdf():
    ...
    def bomba(self, bomba):
        ...
        self.c.drawString(11.6*cm, 495, "Presión de vapor @ T.B.:  "+ config.representacion(presion) + config.Configuracion("Pressure").text())
        self.c.drawString(11.6*cm, 480, "Viscosidad @ T.B.:  "+ config.representacion(viscosidad) + config.Configuracion("Viscosity").text())
...
#               numero, densidad_gas, densidad_solido, viscosidad, caudal, solidos_entrada, solidos_salida, rendimiento, delta_P, Dc, Bc, Hc, Jc, Lc, Zc, De, Sc, caudal_unitario, modelo, f_instal, indice, c_adqui, c_instal):
        ...
        self.c.drawCentredString(11.5*cm, 629, "General")
        self.c.setFont("Helvetica", 8)
        self.c.drawString(3.6*cm, 615, "Temperatura:  " + config.representacion(ciclon.entrada.T.config())+config.Configuracion("Temperature").text())
        self.c.drawString(3.6*cm, 600, "Fluido:  " + fluido)
        self.c.drawString(3.6*cm, 585, "Sólidos:  " + solidos)
        self.c.drawString(9.2*cm, 600, "Densidad @ T.P.:  " + config.representacion(ciclon.entrada.Gas.rho.config("DenGas"))+config.Configuracion("Density", "DenGas").text())
        self.c.drawString(14.2*cm, 600, "Viscosidad @ T.P.:  " + config.representacion(ciclon.entrada.Gas.mu.config())+config.Configuracion("Viscosity").text())
        self.c.drawString(9.2*cm, 585, "Densidad @ T.P.:  " + config.representacion(ciclon.entrada.solido.rho.config("DenLiq"))+config.Configuracion("Density", "DenLiq").text())
        self.c.drawString(3.6*cm, 570, "Caudal:  " + config.representacion(ciclon.entrada.Q.config("QGas"))+config.Configuracion("VolFlow", "QGas").text())
        self.c.drawString(11.6*cm, 570, "Eficiencia:  " + config.representacion(ciclon.rendimiento))
        self.c.drawString(3.6*cm, 555, "Concentración sólidos entrada:  " + config.representacion(ciclon.entrada.solido.caudal.config())+config.Configuracion("MassFlow").text())
        self.c.drawString(11.6*cm, 555, "Concentración sólidos salida:  " + config.representacion(ciclon.SalidaAire.solido.caudal.config())+config.Configuracion("MassFlow").text()) 
        self.c.drawString(3.6*cm, 540, "Perdida de presión:  " + config.representacion(ciclon.DeltaP.config())+config.Configuracion("Pressure").text())
        ...
        self.c.drawString(3.7*cm, 495, "Caudal Unitario:  " + config.representacion(ciclon.entrada.Q.config("QGas"))+config.Configuracion("VolFlow", "QGas").text())
        self.c.drawString(6.8*cm, 465, "Dc:  " + config.representacion(ciclon.Dc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 450, "Bc:  " + config.representacion(ciclon.Bc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 435, "Hc:  " + config.representacion(ciclon.Hc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 420, "Jc:  " + config.representacion(ciclon.Jc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 405, "Lc:  " + config.representacion(ciclon.Lc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 390, "Zc:  " + config.representacion(ciclon.Zc.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 375, "De:  " + config.representacion(ciclon.De.config())+config.Configuracion("Length").text())
        self.c.drawString(6.8*cm, 360, "Sc:  " + config.representacion(ciclon.Sc.config())+config.Configuracion("Length").text())
