...
#        return unidades.Length((8430.153*log(101325/self.P))/(1+0.095*log(101325/self.P)))
    ...
    def Isocora_T(self, H, v):
        ...
        return unidades.Temperature(self.P.atm*V.lg*self.aire.M/R_atml /
                                    (1+H*self.aire.M/self.agua.M))

    def definirPunto(self, modo, tdb=None, twb=None, tdp=None, H=None,
                     HR=None, volumen=None, entalpia=None):
        """
        ...
        if modo == 0:
            ...
            def f(H):
                return self.Humedad_Absoluta(twb)-H - \
                    self.Calor_Especifico_Humedo(tdb, H)/Hv*(tdb-twb)
...
class Punto_Psicrometrico(object):
    def __init__(self, P=1, caudal=None, modo=None, tdb=None, twb=None,
                 tdp=None, H=None, HR=None, volumen=None, entalpia=None):
    ...
    def definirPunto(self, modo, tdb=None, twb=None, tdp=None, H=None, HR=None,
                     volumen=None, entalpia=None):
        """
        modo: datos conocidos
            0   -   tdb, H
            1   -   tdb, HR
        ...
        if modo == 0:
            ...
            def f(H):
                return self.AireHumedo.Humedad_Absoluta(twb)-H - \
                    self.AireHumedo.Calor_Especifico_Humedo(tdb, H)/Hv*(tdb-twb)
            self.H = fsolve(f, 0.001)
            self.Tdp = self.AireHumedo.Tdp(self.H)
            self.HR = self.H/self.Hs*100
            self.entalpia = self.AireHumedo.Entalpia(tdb, self.H)
