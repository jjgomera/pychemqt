...
data = [(QApplication.translate("pychemqt", "Temperature"), "T"),
        ...
        (QApplication.translate("pychemqt", "Specific isochoric heat capacity"), "cv"),
        (QApplication.translate("pychemqt", "Specific isobaric heat capacity"), "cp"),
        (QApplication.translate("pychemqt", "Heat capacities ratio"), "cp_cv"),
        ...
        (QApplication.translate("pychemqt", "Volumetric Expansitivy"), "alfav"),
        (QApplication.translate("pychemqt", "Isotermic compresibility"), "kappa"),
        (QApplication.translate("pychemqt", "Relative pressure"), "alfap"),
        (QApplication.translate("pychemqt", "Isothermal stress"), "betap"),
        (QApplication.translate("pychemqt", "Isentropic temperature-pressure"), "betas"),
        (QApplication.translate("pychemqt", "Joule-Thomson coefficient"), "joule"),
        (QApplication.translate("pychemqt", "Gruneisen parameter"), "Gruneisen"),
...
#        Z_rho     -   (Z-1) over the density, m³/kg
        ...
        (QApplication.translate("pychemqt", "Specific heat input"), "hInput"),
        ...
        (QApplication.translate("pychemqt", "Prandtl number"), "Pramdt"),
        (QApplication.translate("pychemqt", "Dielectric constant"), "epsilon"),
        (QApplication.translate("pychemqt", "Ideal gas Specific volume"), "v0"),
        (QApplication.translate("pychemqt", "Ideal gas Density"), "rho0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific enthalpy"), "h0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific internal energy"), "u0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific entropy"), "s0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific Helmholtz free energy"), "a0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific Gibbs free energy"), "g0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific isobaric heat capacity"), "cp0"),
        (QApplication.translate("pychemqt", "Ideal gas Specific isochoric heat capacity"), "cv0"),
...
class MEoS(_fase):
    ...
    def calculo(self):
        ...
        if x == 0:
            ...
            self.v = unidades.SpecificVolume(x*self.Gas.v+(1-x)*self.Liquid.v)
            self.rho = unidades.Density(1./self.v)

            self.h = unidades.Enthalpy(x*self.Gas.h+(1-x)*self.Liquid.h)
            self.s = unidades.SpecificHeat(x*self.Gas.s+(1-x)*self.Liquid.s)
            self.u = unidades.Enthalpy(x*self.Gas.u+(1-x)*self.Liquid.u)
            self.a = unidades.Enthalpy(x*self.Gas.a+(1-x)*self.Liquid.a)
            self.g = unidades.Enthalpy(x*self.Gas.g+(1-x)*self.Liquid.g)

            self.Z = unidades.Dimensionless(x*self.Gas.Z+(1-x)*self.Liquid.Z)
            self.f = unidades.Pressure(x*self.Gas.f+(1-x)*self.Liquid.f)
            self.Z_rho = unidades.SpecificVolume(x*self.Gas.Z_rho+(1-x)*self.Liquid.Z_rho)
            self.IntP = unidades.Pressure(x*self.Gas.IntP+(1-x)*self.Liquid.IntP)
