...
class UI_confUnits_widget(QtGui.QWidget):
    def __init__(self, config=None, parent=None):
        ...
        systems = QtGui.QGroupBox(QtGui.QApplication.translate("pychemqt", "Systems of measurement"))
        systems.setSizePolicy(QtGui.QSizePolicy.Preferred, QtGui.QSizePolicy.Fixed)
        layout.addWidget(systems,1,1,1,2)
        lytSystems = QtGui.QHBoxLayout(systems)
        self.SI = QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "SI"))
        self.SI.toggled.connect(partial(self.load, "si"))
        lytSystems.addWidget(self.SI)
        self.AltSI = QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "Alt SI"))
        self.AltSI.toggled.connect(partial(self.load, "altsi"))
        lytSystems.addWidget(self.AltSI)
        self.English = QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "English"))
        self.English.toggled.connect(partial(self.load, "english"))
        lytSystems.addWidget(self.English)
        self.Metric = QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "Metric"))
        self.Metric.toggled.connect(partial(self.load, "metric"))
        lytSystems.addWidget(self.Metric)
        self.CGS = QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "CGS"))
        ...
        for i, magnitud in enumerate(unidades._magnitudes[:-1]):
            self.tabla.setVerticalHeaderItem(i, QtGui.QTableWidgetItem(magnitud[1]))
            self.tabla.setRowHeight(i,24)
            combo=QtGui.QComboBox()
            if magnitud[0]=="Currency":
                for texto, unidad in zip(magnitud[2].__text__, magnitud[2].__units__):
                    combo.addItem(QtGui.QIcon(QtGui.QPixmap(os.environ["pychemqt"]+"/images/flag/%s.gif" % unidad)), texto+" - "+unidad)
            else:
        ...
        self.tabla.setFixedWidth(self.tabla.verticalHeader().sizeHint().width()+self.tabla.columnWidth(0)+20)

        self.nombre = QtGui.QLineEdit()
        self.nombre.textChanged.connect(self.nameChanged)
        layout.addWidget(self.nombre,3,2)
        self.Guardar = QtGui.QPushButton(QtGui.QIcon(QtGui.QPixmap(os.environ["pychemqt"]+"/images/button/fileSave.png")), QtGui.QApplication.translate("pychemqt", "Save profile"))
        ...
        self.Cargar = QtGui.QPushButton(QtGui.QIcon(QtGui.QPixmap(os.environ["pychemqt"]+"/images/button/fileOpen.png")), QtGui.QApplication.translate("pychemqt", "Load profile"))
