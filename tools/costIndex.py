...
indiceBase=["Jan-1982", 313.95, 336.19, 326.01, 312.03, 383.18, 297.63, 421.1, 235.42, 338.2, 263.92, 290.13, 303.26]
...
class Ui_CostIndex(QtGui.QDialog):
    def __init__(self, parent=None):
        super(Ui_CostIndex, self).__init__(parent)
        self.setWindowTitle(QtGui.QApplication.translate("pychemqt", "Cost Index"))
        self.custom=True
        layout = QtGui.QGridLayout(self)
        self.fecha = QtGui.QComboBox()
        layout.addWidget(self.fecha,1,1,1,3)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "CE INDEX")),2,1,1,2)
        self.index = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.index,2,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Equipments")),3,1,1,2)
        self.equipos = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.equipos,3,3,1,1)
        layout.addItem(QtGui.QSpacerItem(30,0,QtGui.QSizePolicy.Fixed,QtGui.QSizePolicy.Fixed),4,1,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Heat exchangers & Tanks")),4,2,1,1)
        self.cambiadores_calor = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.cambiadores_calor,4,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Process machinery")),5,2,1,1)
        self.maquinaria = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.maquinaria,5,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Pipe, valves & fittings")),6,2,1,1)
        self.tuberias = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.tuberias,6,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Process instruments")),7,2,1,1)
        self.instrumentos = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.instrumentos,7,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Pumps & compressors")),8,2,1,1)
        self.bombas = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.bombas,8,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Electrical equipments")),9,2,1,1)
        self.equipos_electricos = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.equipos_electricos,9,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Structural supports & misc")),10,2,1,1)
        self.soportes = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.soportes,10,3,1,1)

        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Construction labor")),11,1,1,2)
        self.construccion = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.construccion,11,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Buildings")),12,1,1,2)
        self.edificios = Entrada_con_unidades(float, width=70, decimales=1)
        layout.addWidget(self.edificios,12,3,1,1)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Engineering & supervision")),13,1,1,2)
...
class CostData(QtGui.QWidget):
    """Conjunto de widgets comunes a todas las secciones de costos"""
    valueChanged = QtCore.pyqtSignal(QtCore.QString, float)
    def __init__(self, equipment, parent=None):
        ...
        gridLayout.addItem(QtGui.QSpacerItem(20,20,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),1,0,1,7)
        gridLayout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Instalation factor:")),2,0,1,1)
        self.factorInstalacion = Entrada_con_unidades(float, spinbox=True, decimales=1, step=0.1, width=50, value=factor)
        self.factorInstalacion.valueChanged.connect(partial(self.valueChanged.emit, "f_install"))
        gridLayout.addWidget(self.factorInstalacion,2,1,1,1)
        gridLayout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Base index:")),2,4,1,1)
        self.indiceBase = Entrada_con_unidades(float, readOnly=True, value=indiceBase[self.indice], decimales=1)
        gridLayout.addWidget(self.indiceBase,2, 5,1,1)
        self.textLabel1_13_2 = QtGui.QLabel()
        self.textLabel1_13_2.setText(QtGui.QApplication.translate("pychemqt", "Current index:"))
        gridLayout.addWidget(self.textLabel1_13_2,3,4,1,1)
        self.indiceActual = Entrada_con_unidades(float, readOnly=True, colorReadOnly="white", value=indiceActual[self.indice], decimales=1)
        gridLayout.addWidget(self.indiceActual,3,5,1,1)
        self.costIndex = QtGui.QToolButton()
        ...
        gridLayout.addItem(QtGui.QSpacerItem(20,20,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),4,0,1,7)
    
    def on_costIndex_clicked(self):
        dialog = Ui_CostIndex()
        if dialog.exec_():
            with open(config.conf_dir+"CostIndex.dat", "r") as archivo:
                self.indiceActual.setValue(float(archivo.readlines()[self.indice][:-1]))
    ...
    @property
    def values(self):
        return self.factorInstalacion.value, self.indiceBase.value, self.indiceActual.value
