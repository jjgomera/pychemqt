...
def f_rib_Naphon(Re, Di, p, Xi):
    ...
    """
    # Eq 11
    f = 7.85 / Re**0.21 * (Xi/Di)**1.68 / (p/Di)**0.54
...
@refDoc(__doi__, [1])
def Nu_rib_Naphon(Re, Pr, Di, p, Xi):
    ...
    """
    # Eq 8
    Nu = 44.26 * (Re-1500)**0.27 / Pr**0.26 * (Xi/Di)**0.89 / (p/Di)**0.96
...
class Rib(CallableEntity):
    ...
    def Nu(self, Re, Pr, Di):
        ...
        Nu = Nu_rib_Naphon(Re, Pr, Di, self.p, self.Xi)
        return Nu

    def f(self, Re, Di):
        """Calculate friction factor"""
        f = f_rib_Naphon(Re, Di, self.p, self.Xi)
        return f
...
class UI_Rib(ToolGui):
    ...
    def loadUI(self):
        ...
        self.Xi = Entrada_con_unidades(Length, "Thickness")
        self.Xi.valueChanged.connect(partial(self.changeParams, "Xi"))
        lyt.addWidget(self.Xi, 3, 2)
