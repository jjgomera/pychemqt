...
##########################################################
### Diálogo de definición de unidades de interaccion con hojas de calculo, UI_streadsheet ###
##########################################################
...
class UI_equipment(UI_equip):
    """Diálogo de definición de cunidades de interaccion con hojas de calculo"""
...
#            patrones.append(QtGui.QApplication.translate("pychemqt", "Microsoft Excel 97/2000/XP/2003 XMLL")+ " (*.xls)")
    ...
    def rellenarInput(self):
        ...
        if self.Equipment.status:
            ...
            self.datamap.itemDelegateForRow(0).setItemsByIndex(3, self.Equipment.sheets)
        ...
        if self.Equipment.kwargs["datamap"]:
            for i, data in enumerate(self.Equipment.kwargs["datamap"]):
                self.datamap.addRow()
                self.datamap.itemDelegateForRow(i).setItemsByIndex(0, self.entitys)
                self.datamap.itemDelegateForRow(i).setItemsByIndex(3, self.Equipment.sheets)
                self.datamap.setItem(i, 0, QtGui.QTableWidgetItem(data["entity"]))
                self.datamap.setItem(i, 1, QtGui.QTableWidgetItem(data["property"]))
                self.datamap.setItem(i, 2, QtGui.QTableWidgetItem(data["unit"]))
                self.datamap.setItem(i, 3, QtGui.QTableWidgetItem(data["sheet"]))
                self.datamap.setItem(i, 4, QtGui.QTableWidgetItem(data["cell"]))
            self.datamap.itemDelegateForRow(self.datamap.rowCount()-1).setItemsByIndex(0, self.entitys)
            self.datamap.itemDelegateForRow(self.datamap.rowCount()-1).setItemsByIndex(3, self.Equipment.sheets)
        self.datamap.blockSignals(False)
