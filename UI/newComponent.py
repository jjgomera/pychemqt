...
from lib.compuestos import Joback, Constantinou_Gani, Wilson_Jasperson, Marrero_Pardillo, Elliott, Ambrose
from lib import sql


class newComponent(QtGui.QDialog):
    """Clase general para los dialogos de definicion de nuevos componentes"""
    
    def loadUI(self):
        """Crea los widgets comunes de la parte inferior de las clases hijas"""
        layoutBottom=QtGui.QHBoxLayout()
        self.status=Status()
        layoutBottom.addWidget(self.status)
        self.buttonShowDetails=QtGui.QPushButton(QtGui.QApplication.translate("pychemqt", "Show Details"))
        ...
        self.buttonBox = QtGui.QDialogButtonBox(QtGui.QDialogButtonBox.Cancel|QtGui.QDialogButtonBox.Save)
    ...
    def save(self):
        """Guarda el componente creado en la base de datos"""
    ...
    def calculo(self, **kwargs):
        ...
        self.buttonBox.button(QtGui.QDialogButtonBox.Save).setEnabled(self.unknown.status)
        
    def showDetails(self):
        """Muestra el diologo con los detalles del nuevo compuesto creado"""
...
class Ui_Contribution(newComponent):
    """Clase que modela el dialogo de definición de nuevos componentes en los métodos de contribución de grupos"""
    ...
    def __init__(self, metodo, parent=None):
        ...
        self.setWindowTitle(QtGui.QApplication.translate("pychemqt", "Select the component group for method")+" "+metodo)
        ...
        self.Grupos.setHorizontalHeaderItem(1, QtGui.QTableWidgetItem(QtGui.QApplication.translate("pychemqt", "Group")))
        ...
        layout.addItem(QtGui.QSpacerItem(20,20,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),5,1,1,1)
        if metodo in ["Constantinou", "Wilson"]:
            self.Order1=QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "1st order"))
            self.Order1.setChecked(True)
            self.Order1.toggled.connect(self.Order)
            layout.addWidget(self.Order1, 6, 3)
            self.Order2=QtGui.QRadioButton(QtGui.QApplication.translate("pychemqt", "2nd order"))
            layout.addWidget(self.Order2, 7, 3)
            
        if metodo=="Wilson":
            layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Rings")), 8, 3)
        ...
        if metodo=="Marrero":
            layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Atoms")), 8, 3)
        ...
        if metodo=="Ambrose":
            layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Platt number")), 8, 3)
            self.Platt=QtGui.QSpinBox()
            self.Platt.setToolTip(QtGui.QApplication.translate("pychemqt", "The Platt number is the number of pairs of carbon atoms which are separated \nby three carbon-carbon bonds and is an indicator of the degree of branching in the molecule.\nThe Platt number of an n-alkane is equal to the number of carbons minus three"))
            self.Platt.valueChanged.connect(partial(self.changeParams, "platt"))
            layout.addWidget(self.Platt, 9, 3)

        layout.addItem(QtGui.QSpacerItem(20,20,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),10,1,1,1)
        layout.addItem(QtGui.QSpacerItem(20,20,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),11,0,1,4)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Name")), 12, 0)
        ...
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Molecular Weight")), 13, 0)
        ...
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Boiling point")), 14, 0)
        ...
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Specific Gravity")), 15, 0)
    ...
    def add(self):
        ...
        if grupo not in self.grupo:
            ...
            self.Grupos.item(indice, 0).setTextAlignment(QtCore.Qt.AlignRight|QtCore.Qt.AlignVCenter)
            self.Grupos.setItem(indice, 1, QtGui.QTableWidgetItem(grupo))
            self.Grupos.item(indice, 1).setFlags(QtCore.Qt.ItemIsSelectable|QtCore.Qt.ItemIsEnabled)
            self.Grupos.setRowHeight(indice, 20)
        else:
            indice=self.grupo.index(grupo)
            self.contribucion[indice]+=1
            self.Grupos.item(indice, 0).setText(str(int(self.Grupos.item(indice, 0).text())+1))
        self.calculo(**{"group": self.indices, "contribution": self.contribucion})
...
class Definicion_Petro(newComponent):
    """Clase que define el dialogo de definición de fracciones petrolíferas"""
    ViewDetails=View_Petro
    def __init__(self, parent=None):
        super(Definicion_Petro, self).__init__(parent)
        self.setWindowTitle(QtGui.QApplication.translate("pychemqt", "Petrol component definition"))
        layout = QtGui.QGridLayout(self)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Name")),1,1)
        self.nombre=QtGui.QLineEdit()
        self.nombre.textChanged.connect(partial(self.changeParams, "name"))
        layout.addWidget(self.nombre,1,2,1,4)
        layout.addItem(QtGui.QSpacerItem(10,10,QtGui.QSizePolicy.Fixed,QtGui.QSizePolicy.Fixed),2,1,1,2)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Boiling point")),3,1)
        self.Tb=Entrada_con_unidades(Temperature)
        self.Tb.valueChanged.connect(partial(self.changeParams, "Tb"))
        layout.addWidget(self.Tb,3,2)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Molecular Weight")),4,1)
        self.M=Entrada_con_unidades(float, textounidad="g/mol")
        self.M.valueChanged.connect(partial(self.changeParams, "M"))
        layout.addWidget(self.M,4,2)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Specific Gravity")),5,1)
        self.SG=Entrada_con_unidades(float)
        self.SG.valueChanged.connect(partial(self.changeParams, "SG"))
        layout.addWidget(self.SG,5,2)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "API gravity")),6,1)
        ...
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Refraction index")),11,1)
        self.n=Entrada_con_unidades(float)
        self.n.valueChanged.connect(partial(self.changeParams, "n"))
        layout.addWidget(self.n,11,2)
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Huang parameter")),12,1)
        ...
        layout.addWidget(QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Carbons number")),19,1)
        self.carbonos=Entrada_con_unidades(int, width=50, spinbox=True, step=1, start=7, min=5, max=100)
        self.N.valueChanged.connect(partial(self.changeParams, "Nc"))
        layout.addWidget(self.carbonos,19,2)

        layout.addItem(QtGui.QSpacerItem(10,10,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),3,3,15,1)
        self.checkCurva=QtGui.QCheckBox(QtGui.QApplication.translate("pychemqt", "Define destillation curve"))
        ...
        self.textoPresion=QtGui.QLabel(QtGui.QApplication.translate("pychemqt", "Pressure"))
        ...
        self.curvaDestilacion=Tabla(2, filas=1, horizontalHeader=["%dist", "Tb, "+Temperature.text()], verticalHeader=False, dinamica=True)
        ...
        self.checkBlend=QtGui.QCheckBox(QtGui.QApplication.translate("pychemqt", "Blend if its necessary"))
        ...
        layout.addItem(QtGui.QSpacerItem(20,20,QtGui.QSizePolicy.Fixed,QtGui.QSizePolicy.Fixed),20,1,1,2)
        self.checkCrudo=QtGui.QCheckBox(QtGui.QApplication.translate("pychemqt", "Use petrol fraction from list"))
        ...
        self.crudo.addItem("")
        for i in crudo[1:]:
            self.crudo.addItem(i[0]+" ("+i[1]+")"+"   API: "+str(i[3])+"  %S: "+str(i[4]))
        self.crudo.currentIndexChanged.connect(partial(self.changeParams, "indice"))
        layout.addWidget(self.crudo,23,1,1,5)
        layout.addWidget(QtGui.QLabel("Pseudo C+"),24,1)
        self.Cplus=Entrada_con_unidades(int, width=50, spinbox=True, step=1, min=6)
        ...
        layout.addItem(QtGui.QSpacerItem(5,5,QtGui.QSizePolicy.Expanding,QtGui.QSizePolicy.Expanding),29,1,1,2)
